 ...
 * @author mindori
 */
public class MxTypeDecoder {
	private Logger logger = LoggerFactory.getLogger(MxTypeDecoder.class);
	private static final int Preference_Length = 2;
	...
	public DnsAnswer decodeBody_Answer(byte[] replyBuffer, ByteBuffer buffer) {
		...
		int mailLength = dataLength - Preference_Length;
		DnsNameParser dnsNameParser = new DnsNameParser();
		String mailExchange = dnsNameParser
				.parse(replyBuffer, buffer, mailLength);
		answer.setMailExchange(mailExchange);		
		logger.info("Responsible authority's mailbox : " + mailExchange);
