...
public class TextHttpServlet extends HttpServlet {
	final Logger logger = LoggerFactory.getLogger(TextHttpServlet.class.getName());
	private static final long serialVersionUID = 1L;

	private TextHttpServiceApi manager;

	public TextHttpServlet(TextHttpServiceApi manager) {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		logger.debug("text servlet pathinfo: " + req.getPathInfo());
		...
		try {
			String text = (String) manager.invokeTextMethod(filterId, methodName, req);
			if (text == null) {
				logger.warn("text servlet null returned.");
			}
			...
			try {
				...
				String text = (String) manager.invokeTextMethod(filterId, methodName, params);
				if (text == null) {
					logger.warn("text servlet null returned.");
				}
				resp.getWriter().print(text);
